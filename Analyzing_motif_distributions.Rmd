...
author: "Chante Bethell and Stephanie J. Spielman, PhD"
...
The plot following each table represents a heatmap of proteins organized by their distribution in the disordered and ordered regions for specific motifs. The AP2 adaptor complex is known to recognize the $YXX\Phi$ and $X[DE]XXXL[LI]$ endocytic signal motifs within transmembrane cargoes (11). In order to study each motif separately, heatmaps and tables were produced for each specific motif. Each rectangle on the heatmaps is labeled with a white number representing the number of proteins that have the same number of motif instances in the ordered and disordered regions. The rectangles found higher on the plots (above the green one to one line) represent the proteins that are enriched in the disordered regions. These are the proteins that should be targeted based on our hypothesis. A different R package was used to display the results for the motif $[ST]XXXL[LI]$ for better visualization. 
